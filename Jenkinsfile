node {
  ...
  stage ('Build Docker Image') {
   sh 'ssh -p2244 pkbot@pkbotnode  docker -H  $pkbot_docker_connect build -t $hubot_image_name $pkbot_home/'
   sleep 5 
  }
  stage ('Build Docker Container') {
   sh 'ssh -p2244 pkbot@pkbotnode  chmod +x $pkbot_home/remove-container.sh' 
